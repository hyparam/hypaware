...
which is what makes "a never-attached machine's own providers are never
edited in either file" hold by construction instead of by a caller-side gate.
These caches do not self-heal, so retryability matters: a detach whose
settings half landed but whose cache half failed is finished by rerunning it,
because the residue still carries the marker that identifies it. Backup keys
written by earlier versions are left as they are; nothing reads or writes
that key anymore.
