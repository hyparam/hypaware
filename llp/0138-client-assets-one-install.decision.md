...
**Extended-by:** LLP 0219 (the same materializer also removes what this version no longer contributes; #marker-undo's "manual copies record no marker" is answered with an install ledger), LLP 0247 (#marker-undo's "`installed_assets` is the evidence that something was applied" is only the asset half; a settings-only attach records `prior_done` instead)
