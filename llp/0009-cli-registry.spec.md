...
**Extended-by:** LLP 0406 (redundant CLI flag removal), LLP 0267 (#d1: top-level help reads the manifest while group help reads the registry, so the two are compared rather than assumed to agree), LLP 0293 (a registration's `usage` line and its parser are one declaration, and every visible core command refuses input it does not know with exit 2)
