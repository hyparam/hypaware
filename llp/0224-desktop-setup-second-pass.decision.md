...
> `action_attach.js` `desired()`, so declaring one would re-open the
> reconciler's attach-on-join path. A retired client is skipped by
> `client_attach_missing` outright, and residue is reported by its own
> check.
