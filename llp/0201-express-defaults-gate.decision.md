...
> Record and sync everything
    Record AI logs from Claude Code and Codex.
  Customize
...
disclosure (what accepting does) and the evidence (what was found), in the
one line the happy path is guaranteed to read. The names are the pick
...
forwards, so the labels drop to "Record everything" / "Record AI logs from
Claude Code." and the Customize summary to "Choose what to record." The
