...
### One `api_request` record arbitrates, so order does not matter {#claim-order-arbitrates}
...
makes the question moot: the turn is counted once whatever order the events
arrive in. In the order Claude Code actually exports (`api_request`, then
`api_response_body`, then `assistant_response`) the body row claims it and the
placement is the one this decision wants; in the reverse order the text row
claims it and the placement is the pre-fix one, counted once. The body's own
...
- The per-request `cost_usd`, `duration_ms`, and `speed` that ride the same
  ...
  tool-calling turn they now sit on a body-derived row, which is the row that
  can be collapsed away in favour of the sweep's copy - so hyparam/hypaware#1472
  (a collapsed row losing the OTEL-only fields the sweep has no source for) now
  reaches text-bearing tool turns as well as the text-less ones it already
  reached. Splitting the record so the tokens and the cost land on different
  rows would trade that for a worse shape (one turn's accounting on two rows),
  so it is left to #1472 to settle where those fields belong.
