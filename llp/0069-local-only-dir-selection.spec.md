...
| session opt-out ([LLP 0066](./0066-session-opt-out.spec.md)) | one client session | ephemeral drop (not recorded) | in-memory | `/hypaware-ignore` skill. **Superseded-by: [LLP 0212](./0212-session-opt-out-is-a-cli-verb.decision.md#cli-is-the-verb)** (the skill retired 2026-08-12; authored with `hyp session ignore`). |
