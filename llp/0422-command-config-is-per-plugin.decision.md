...
**Related:** LLP 0004, LLP 0009, LLP 0033, LLP 0034, LLP 0421
...
- **`sinks{}`.** An instance's `config` is the operator's settings for the
  plugins the instance names (`writer` + `destination`, or `plugin`), and the
  kernel already hands it to exactly those through `SinkCreateContext`. So an
  instance keeps its `config` for the plugins it names and loses it for
  everyone else, while the instance itself stays visible. A `hyp` command that
  reported an empty sink list would be reporting something untrue.
...
- A plugin's verb projection now also receives that plugin's `capabilities`,
  ...
  not a new reach.
- **No bundled plugin loses anything it reads today.** Surveyed across
  `hypaware-core/plugins-workspace/`, `src/` and `bin/`, every command body and
  verb operation that reads `ctx.config` reads either its own `plugins[]`
  section (`@hypaware/claude`'s hook cap, the gateway's `session` commands) or
  passes the whole config to `executeQuerySql`, which forwards it to
  `discoverPartitions`; no bundled dataset reads it.
...
- Cost is one shallow object, one array of `plugins[]` length and one record of
  ...
  command. Nothing per-record, nothing per-row, and core's path allocates
  nothing new at all.
