...
keychain still trusts it. `hyp client detach claude` deliberately keeps the CA and
the trust in place, so re-attaching later does not ask for your password
again; `hyp client detach claude --purge` and `hyp daemon uninstall` remove the CA
