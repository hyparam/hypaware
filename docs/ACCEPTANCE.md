...
**What it proves:** Codex CLI and Desktop operate independently of the HypAware
daemon and their native rollouts reach `ai_gateway_messages` on its next sweep.
Requires a real Mac with both clients installed and signed in, and the candidate
HypAware package. Run against disposable client and HypAware homes where the
client supports them. Never claim the fixture smoke proves upstream formats.

1. Enable `@hypaware/codex` with no `capture_mode` override. Run
   `hyp client attach codex`. Confirm `$CODEX_HOME/config.toml` (default
   `~/.codex/config.toml`) contains no HypAware-managed provider route and that
   unrelated provider/model settings survived. On an upgrade, confirm the
   previous provider selection was restored. Restart both clients.
2. Stop the HypAware daemon. In each client, send a distinct harmless prompt
   that invokes a tool and completes. Both must succeed with the daemon stopped.
   Record both client versions, session IDs and the rollout `originator`s.
3. Start the daemon and wait for its next one-minute sweep. Query each session:

   ```sh
   hyp query sql "select entrypoint, role, part_type, content_text, system_text, tools, attributes from ai_gateway_messages where session_id = '<session-id>' order by message_index, part_index"
   ```

   Confirm user/assistant messages, tool calls/results, native base/developer
   instructions and usage. `tools` is null for these newly captured sessions.
   Confirm the two entrypoints match the recorded native originators.
4. Wait another sweep without changing the files. Check daemon telemetry for
   `codex.backfill.scan_complete` with `files_read: 0` and `files_unchanged > 0`
   (on a quiet isolated home), and confirm row counts have not grown.
...
Record the versions, native originators, row counts, usage totals, stopped-daemon
...
This procedure tests optional `capture_mode: "gateway"`. Set it explicitly in
the Codex plugin config before attaching; default capture uses native files.
