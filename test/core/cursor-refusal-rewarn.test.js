...
/**
 * The shared stderr capture, narrowed to the one refusal this suite counts.
 *
 * @param {() => void} fn
 * @returns {Promise<string[]>}
 */
function refusalLines(fn) {
  return stderrLinesFrom(fn, 'cursor_table_dir_escapes_partition')
}
...
test('a heal resets the window: a repoisoned partition warns afresh', async () => {
  ...
  try {
    await writeCursor(partition, '../out')
    assert.equal((await refusalLines(() => { tryReadCursorSync(partition) })).length, 1)
    ...
    assert.equal((await refusalLines(() => {
...
test('a changed rejected value warns without waiting out the window', async () => {
  ...
  try {
    ...
    const lines = await refusalLines(() => { tryReadCursorSync(partition) })
...
test('an unchanged standing refusal says it again once the interval passes', async (t) => {
  ...
  try {
    ...
    assert.equal((await refusalLines(() => { tryReadCursorSync(partition) })).length, 1)
    assert.equal((await refusalLines(() => { tryReadCursorSync(partition) })).length, 0, 'inside the window: quiet')
...
test('two poisoned partitions of one dataset warn independently', async () => {
  ...
  try {
    ...
    const first = await refusalLines(() => {
    ...
    const second = await refusalLines(() => {
      assert.equal(tryReadCursorSync(b), null)
      assert.equal(tryReadCursorSync(a), null)
    })
