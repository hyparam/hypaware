...
test('runPickerWalkthrough drives the TUI multiselect end-to-end when stdin+stdout are TTYs', async () => {
  ...
  try {
    ...
    // Sources prompt. The rendered rows are claude, claude-desktop, codex,
    // otel: the raw API rows are hidden (LLP 0202). Move down three times
...
test('runPickerWalkthrough falls back to the legacy numbered prompt under HYP_NO_TUI=1', async () => {
  ...
  // TTY probe. One answer: source '4' (otel; the raw API rows are hidden,
