 ...
 *
 * A runtime whose `JSON.stringify` is iterative has no such depth at all, and
 * Node 26 is one: it stringifies every depth this probe builds. So the ceiling
 * is what bounds the cost of proving that negative. It sits at 1 << 15, well
 * clear of the deepest stack measured above, because the loop runs to the
 * ceiling on exactly the runtimes that will never fail: uncapped it spends
 * ~570ms and ~210MB of RSS on Node 26 building a million-deep array to learn
 * nothing, and capped it costs ~14ms and allocates nothing that matters on
 * every runtime alike.
 *
 * @returns {number} the depth to use, or 0 if nothing in range defeated stringify
 */
function depthPastStringify() {
  for (let depth = 1024; depth <= 1 << 15; depth *= 2) {
...
test('an id too deep for JSON.stringify defeats the backstop, and says so rather than crashing', async t => {
  // The limit of "serializable by construction". Where V8 parses deeper than it
  // stringifies, a structural id nested past a few thousand levels arrives
  ...
  //
  // The gap is the runtime's, not this code's, so the probe decides whether
  // there is anything to pin. Node 22 and 24 have it and run the check; Node 26
  // stringifies iteratively and has no such depth, and there the backstop is
  // total and this skips rather than asserting a limit that is not there.
  const depth = depthPastStringify()
  if (depth === 0) {
    t.skip('this runtime stringifies as deep as it parses, so the backstop has no unanswerable id')
    return
  }
