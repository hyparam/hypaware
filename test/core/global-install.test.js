...
test('PATH guidance ignores npm temporary bins and quotes paths without editing the shell', async (t) => {
  ...
  assert.equal(available.text(), '', 'any hyp on PATH is enough; which copy it is stays unchecked')
  await fs.unlink(path.join(binDir, 'hyp'))
  writeCliPathGuidance(binPath, { PATH: binDir }, available)
  assert.match(available.text(), /hyp is not on your shell PATH/, 'hypaware alone does not prove the hyp alias')
})
