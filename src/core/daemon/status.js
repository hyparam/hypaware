...
export async function collectHypAwareStatus(opts = {}) {
  ...
  for (const entry of reportedFailures) {
    ...
    diagnostics.push({
      ...
      // off a live daemon's snapshot. The two part company where it costs most.
      // A plugin only the gateway's storage proxy defeats activates normally
      // in this process, so the listing would print it active while this line
      // says it failed, which is why `runPluginList` closes its own section by
      // sending the operator back here rather than the reverse. What it
      // supplies is the name and the version, already in the message above;
      // what it never supplies is the reason, the thing the operator is
      // missing. That reason is clamped to a sentence here and the commonest
      // real one is a module-resolution error longer than that, so the first
      // repair is the record that kept it whole. A listing would in any case
      // be a read that changes nothing (LLP 0139#repair-must-be-runnable).
