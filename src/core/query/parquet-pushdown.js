 ...
 *
 * @import { BinaryNode, BinaryOp, CastType, ComparisonOp, ExprNode, InValuesNode, SqlPrimitive } from 'squirreling/src/types.js'
...
function convertExpr(node, negate) {
  ...
  if (node.type === 'cast' && TRUTHINESS_PRESERVING_CASTS.has(node.toType)) {
    // A cast at boolean position (`WHERE CAST(a = 1 AS INT)`) keeps its
    // operand's truthiness only for boolean and numeric targets. TEXT does
    // not: the engine renders `a = 1` to the string `'false'`, which is
    // truthy, so unwrapping it pushed a filter that drops rows the query
    // selects. TIMESTAMP does not either, since every Date is truthy. Both
...
/**
 * The cast targets whose result is truthy exactly when their operand is, and
 * so may be unwrapped at boolean position. Complement of the four types
 * squirreling's `isCastType` adds: TEXT, STRING, VARCHAR and TIMESTAMP.
 *
 * @type {Set<CastType>}
 */
...
function extractColumnAndValue(left, right) {
  if (left.type === 'identifier') {
    ...
    if (lit) return { column: left.name, value: coerceBigInt(lit.value), flipped: false }
  } else if (right.type === 'identifier') {
    const lit = staticLiteral(left)
    if (lit) return { column: right.name, value: coerceBigInt(lit.value), flipped: true }
  }
  return { column: undefined, value: undefined, flipped: false }
}

/**
 * Statically evaluate an operand to a constant. Handles plain literals and
 * casts wrapping them, which is the shape squirreling parses a typed literal
 * into: `TIMESTAMP '2026-08-11T00:00:00Z'` is a `cast` node over a string
 * literal, not a literal. Folding it is what lets a day-bounded query push
 * its bound down at all; without it the bound declined, and because `AND` is
 * all-or-nothing the whole predicate declined with it, so every
 * timestamp-bounded query scanned unfiltered.
 *
 * The result is wrapped in `{value}` so a literal NULL (which the caller
 * turns into a never-match) stays distinguishable from "not a constant".
 *
 * @ref LLP 0219#decision [implements]: the fold comes from icebird, the NULL
 * semantics do not; a folded bound is guarded exactly like a plain one
 *
 * @param {ExprNode} node
 * @returns {{ value: SqlPrimitive } | undefined}
 */
...
/**
 * Mirror of squirreling's CAST evaluation over primitive literals. This must
 * stay in lockstep with the engine: a converted filter replaces engine-side
 * WHERE rather than pre-filtering for it, so a value folded differently from
 * the way the engine folds it is a wrong answer, not a lost optimisation.
 * Anything the engine would evaluate to null (an unparseable date, NaN)
 * returns undefined so the predicate declines instead.
 *
...
function foldCast(toType, val) {
  ...
  if (toType === 'TEXT' || toType === 'STRING' || toType === 'VARCHAR') {
    // The engine JSON-stringifies an object operand rather than calling
    // String() on it, so `CAST(TIMESTAMP '...' AS TEXT)` would fold to a
    // different string here than the engine produces. Its stringify helper is
    // not exported, so decline rather than reimplement it.
...
/**
 * Mirror of squirreling's TIMESTAMP cast: numbers are epoch millis, strings go
 * through its `toDate`, which insists on a `YYYY-MM-DD` prefix. `toDate` is
 * not in squirreling's public exports, hence the copy.
 *
 * @param {SqlPrimitive} val
...
function convertInValues(node, negate) {
  ...
  for (const val of node.values) {
    ...
    values.push(coerceBigInt(lit.value))
  }
