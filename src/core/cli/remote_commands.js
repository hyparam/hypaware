...
async function runBrowserLogin(name, { org, host, noBrowser, noForward, noDaemon, force, compact = false, binPath }, ctx, { login, seed, enroll, waitForAttach }) {
  ...
  if (seeded.length === 0) {
    ...
    // @ref LLP 0100#requirements [implements]: R1a - the forwarding line names the target by its configured name and prints no URL; the 'hyp remote list' lookup is dropped from the compact line (LLP 0407, plain words), the long form below still carries it
    // @ref LLP 0387#adjacency [constrained-by]: the compact branch is also the deadline line's half of R1a - drop the name here and the compact privacy block below has none
    if (compact) {
