    ...
    const report = await withSpan(
      ...
      async () => {
        ...
        if (!opts.compactOnly) {
          ...
          totalSnapshotsExpired += expired
          if (expired > 0) {
            snapshotsExpiredCounter.add(expired, { [Attr.DATASET]: part.dataset })
          }
        }
        ...
        if (!opts.expireOnly) {
          ...
          if (shouldCompact && !opts.dryRun) {
            ...
            if (result) {
              ...
              totalCompacted += 1
              compactionsCounter.add(1, { [Attr.DATASET]: part.dataset })
            }
...
/**
 * @param {string} epochDir
 * @param {MaintenanceConfig} cfg
 * @param {MaintenanceOptions} opts
 * @returns {Promise<number>}
 */
async function expireSnapshots(epochDir, cfg, opts) {
  if (!tableExists(epochDir)) return 0
 ...
 * @returns {boolean}
 */
function needsCompaction(epochDir, cfg) {
  const dataFiles = countDataFiles(epochDir)
  if (dataFiles > cfg.compact_file_count) return true

  const totalDataBytes = measureDataDir(epochDir)
 ...
 * @returns {number}
 */
function countDataFiles(epochDir) {
  const dataDir = path.join(epochDir, 'data')
  try {
 ...
 * @returns {number}
 */
function countSnapshots(epochDir) {
  if (!tableExists(epochDir)) return 0
 ...
 * @returns {number}
 */
function measureMetadataDir(epochDir) {
  return measureDir(path.join(epochDir, 'metadata'))
}
 ...
 * @returns {number}
 */
function measureDataDir(epochDir) {
  return measureDir(path.join(epochDir, 'data'))
}
