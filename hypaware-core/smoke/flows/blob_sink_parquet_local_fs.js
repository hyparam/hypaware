 ...
 *     `<config.dir>/<dataset>/source=unknown/source=unknown.<since>-<last>.parquet`
...
export async function run({ harness, expect }) {
  ...
  // The exported partition is the one the spool flush committed, and the
  // fixture rows carry no client identity, so it is `source=unknown`.
  // Incremental sink reads (LLP 0040) embed the exported `[sinceSeq, lastSeq]`
  // ingest-seq range in the filename for idempotent re-PUT, so the blob is
  // `source=unknown.<since>-<last>.parquet` (a first export starts at
  // sinceSeq 0).
  const expectedDir = path.join(destinationDir, DATASET, 'source=unknown')
  const blobNames = (await fs.readdir(expectedDir)).filter((n) => /^source=unknown\.\d+-\d+\.parquet$/.test(n))
  expect.that(
    `destination: exactly one ranged parquet blob under ${expectedDir}`,
    blobNames,
    (names) => Array.isArray(names) && names.length === 1
  )
  const expectedFile = path.join(expectedDir, blobNames[0] ?? 'source=unknown.0-0.parquet')
...
const dataset = {
  ...
  async discoverPartitions(ctx) {
    ...
    // so discovery must go through the kernel's helper instead of guessing a
    // static path. The spool label stays listed too: it is what the driver
    // flushes before it re-discovers (cf. @hypaware/otel).
    const spoolPath = path.join(cacheDir, 'datasets', DATASET, 'all')
    const partitions = [{ dataset: DATASET, partition: { partition: 'all' }, tablePath: spoolPath }]
    for (const part of await discoverCachePartitions(cacheDir, { datasets: [DATASET] })) {
