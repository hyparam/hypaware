...
const dataset = {
  ...
  // Mirror the real @hypaware/otel dataset: rows written with no
  // cachePartitioning declaration spool under '<dataset>/all' but commit
  // under 'source=<client>' on flush, so discovery must list the spool
  // path (for flush) and scan the committed 'source=' partitions.
  discoverPartitions(ctx) {
    ...
    const parts = [
      { dataset: DATASET, partition: { partition: 'all' }, tablePath: path.join(datasetDir, 'all') },
    ]
    let entries = []
    try { entries = fs.readdirSync(datasetDir, { withFileTypes: true }) } catch {}
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name.startsWith('source=')) {
        parts.push({
          dataset: DATASET,
          partition: { partition: entry.name },
          tablePath: path.join(datasetDir, entry.name),
        })
      }
    }
    return parts
