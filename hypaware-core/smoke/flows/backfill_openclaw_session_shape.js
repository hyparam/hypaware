 ...
 *
 * Named for the record SHAPE rather than the client, because
 * `backfill_openclaw_fixture.js` is already taken by LLP 0173's T12 sweep
 * flow (#570), which drives `createBackfillSweepDriver` to prove Lane B's
 * quiesce window and cross-write dedupe. The two flows share the client and
 * nothing else: that one enters through the daemon-facing sweep driver and
 * asserts scheduling, this one enters through `hyp backfill openclaw` and
 * asserts what a record's two levels project to. Neither subsumes the
 * other, so both run.
 *
...
export async function run({ harness, expect }) {
  ...
  if (!obs.tracer.provider) {
    throw new Error(
      'backfill_openclaw_session_shape: tracer provider not installed - expected HYP_DEV_TELEMETRY=1'
    )
  }
  ...
  const fixture = await writeOpenclawSessionFixture({
    ...
    // Backdated four minutes, past the provider's 180000ms default quiesce
    // window (LLP 0172#45-the-quiesce-window, #570). A file written moments
    // ago is deliberately invisible to a backfill scan, because settlement
    // may still be flushing the same turn into the live lane. This flow is
    // about what a two-level record projects to, not about scheduling, so
    // it takes the shortest honest route past that gate rather than
    // overriding `backfill.quiesce_ms` to 0: an aged file exercises the
    // real default the way an operator's own sessions will, and leaves the
    // window itself to `backfill_openclaw_fixture.js`, which exists to
    // assert it. Same four minutes, and the same reason, as
    // `test/plugins/openclaw-backfill.test.js`'s default-window cases.
    mtimeMs: Date.now() - 4 * 60 * 1000,
    records: [
    ...
    await runRoot(
      ...
      async () => {
        ...
        if (loaded.length !== pluginDirs.length) {
          throw new Error(`backfill_openclaw_session_shape: expected ${pluginDirs.length} manifests, got ${loaded.length}`)
        }
        const resolution = await resolveDependencies(loaded.map((l) => l.manifest))
        if (resolution.unsatisfied.length > 0) {
          throw new Error(
            `backfill_openclaw_session_shape: unsatisfied requirements: ${
