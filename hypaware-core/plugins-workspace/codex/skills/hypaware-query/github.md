...
- **The evidence boundary is prospective, and withholding it later is not retroactive.** A session dropped by `.hypignore` supplies no row of its own. Adding `.hypignore` or `ignore` does not erase previously admitted rows or already captured GitHub history. Nor does it stop future capture: a repository that has once entered the local inventory stays in it, so the source keeps polling that repository and keeps writing new `github_events` rows for it, which remain eligible to sync, even after every session that evidenced it has been marked `local-only`, opted out, or purged. The GitHub source's `ignore` list is the only control that stops capture of a repository.
