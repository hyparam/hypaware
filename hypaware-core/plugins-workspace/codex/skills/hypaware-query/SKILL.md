...
- **Column-name traps on `node` and `edge`, measured: the same wrong name in 11 sessions in one month on one machine.** Neither table has a `type` column. The kind of a node is `node_type` (`Session`, `Skill`, `Repo`, ...), its identity is `natural_key`, its display text is `label`; an edge's kind is `edge_type`, and its endpoints are typed by `src_type` and `dst_type`. `where n.type = 'Skill'` fails every time and is then retyped; `where n.node_type = 'Skill'` is the query.
