...
The core query subcommands are `overview`, `schema`, and `sql`; active plugins add `grep`, `graph`, or `vector`. Cache operations live under `hyp cache`. There are no high-level `catalog`/`logs`/`traces`/`metrics` query commands; answer questions with `hyp query sql`, and discover datasets from `hyp cache status`.
