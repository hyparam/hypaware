 ...
 * for the `~/.hyp` default.
 *
 * An omitted `nodeBin` drops the interpreter and `exec`s the CLI path itself,
 * for a `hypBin` `node` cannot load: a pnpm shell script or a volta/asdf shim
 * is a fine thing to exec and a `SyntaxError` to interpret, so prefixing one
 * with an interpreter writes a wrapper that fails on its first run
 * (issue #1811).
 *
 * @ref LLP 0116#helper-contract [implements]: generated no-arg wrapper around `hyp claude-account credential`, self-contained on the env it captures and on the interpreter wherever one is baked
...
export function renderCredentialHelperScript(opts) {
  const command = opts.nodeBin === undefined
    ? [opts.hypBin, ...opts.args]
    : [opts.nodeBin, opts.hypBin, ...opts.args]
...
/**
 * Read back the absolute paths `renderCredentialHelperScript` baked into a
 * wrapper: the CLI entry script it `exec`s, and the interpreter in front of
 * it when there is one. The exact inverse of the renderer, and it lives beside
 ...
 * plugin renders.
 *
 * @ref LLP 0116#helper-contract [implements]: the wrapper is generated as `exec [<nodeBin>] <hypBin> claude-account credential`, so the paths it bakes in are readable back out of it
 * @param {string} script
...
export function parseCredentialHelperScript(script) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    // Which of the renderer's two shapes this is, read off the second token:
    // both bake the CLI as an absolute path and nothing else after it is
    // absolute, the credential args being bare words. A leading slash rather
    // than `path.isAbsolute`, because the line is a POSIX `/bin/sh` command
    // whatever platform reads it back (issue #1811).
    if (!tokens[1].startsWith('/')) return { nodeBin: undefined, hypBin: tokens[0] }
