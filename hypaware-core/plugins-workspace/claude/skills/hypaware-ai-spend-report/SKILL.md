...
description: AI Spend Review for a HypAware server — where tokens go (by user/repo/model/gateway), what work they go on (graph-clustered), and how to spend less (waste scorecard + cost levers). Deduped token volume, never dollars. Saves a dated report under docs/spend-reviews/; queries the server via the hypaware-query skill.
---
...
IMPORTANT: Query the server, not local logs. **Tokens, never dollars** — capture is partial,
so stop at token volume and behavioral proxies. Query mechanics (`--server`, date-pruning the
30s timeout, the SQL dialect, sampling) live in the **hypaware-query** skill; read it first.
...
- **Path:** `docs/spend-reviews/<YYYY-MM-DD>-spend-review.md` (create the dir if needed). Dated
...
- **Then the detail**, in these sections: **Coverage & footprint · Total spend (4 token types)
  ...
  scorecard · Leverage candidates (handoff to hypaware-ai-improvement-report) · Caveats** — plus every query
  you ran.
