...
- **Column-name traps, each one measured in recorded sessions.** There is no `source` column (it is `conversation_source`), no `timestamp` and no `ts` (the event-time column is `message_created_at`), and no `usage` (token counts live under `attributes.usage`). A failed query echoes the real column list: read it instead of guessing a second name.
- **More functions that do not exist**, beyond those above: `json_extract_string`, `split_part`, `now`, `datetime`, `strftime`, `group_concat`, `typeof`, `chr`. There are no date/time functions at all, so derive periods from the STRING `date` column (`substr(date, 1, 7)` for a month).
