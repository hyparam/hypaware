...
- **Capture is agent-evidence-gated by default.** Local `inventory = "session_repos"` captures every repository that appears as `git_remote` on an admitted `ai_gateway_messages` row. `inventory = "all_visible"` is the explicit opt-in to enumerate every repository visible to the authenticated GitHub identity. A server may intersect admitted evidence with configured `repos` and `orgs`. The GitHub source's own `ignore` list is applied in every mode.
- **`.hypignore` is not a repository-level guarantee, so do not describe it as one.** It is resolved from a session's directory, so it bounds evidence only where a session actually ran. It cannot reach a repository that has no local checkout, which is most of what `all_visible` adds. Only the `ignore` class withholds a row: a `local-only` directory still produces an admitted row whose `git_remote` is evidence. When a user wants a repository kept out of GitHub capture, point at the GitHub source's `ignore` list rather than promising `.hypignore` covers it.
