...
- **The source is optional on both local and server hosts.** Query the local graph without `--remote`; use `--remote` for a central graph. **An empty GitHub result usually means the source is not configured on that host, capture has not run, or the graph has not been re-projected since capture, not that the true answer is zero.**
- **Capture is agent-evidence-gated by default.** Local `inventory = "session_repos"` captures every repository that appears as `git_remote` on an admitted `ai_gateway_messages` row. `inventory = "all_visible"` is the explicit opt-in to enumerate every repository visible to the authenticated GitHub identity. A server may intersect admitted evidence with configured `repos` and `orgs`. The GitHub source's own `ignore` list is applied in every mode. These keys belong to `@hypaware/github`, which ships and is configured outside this install, so describe the shape but read that source's own documentation before writing a config block from these names.
- **`.hypignore` is not a repository-level guarantee, so do not describe it as one.** It is resolved from a session's directory, so it bounds evidence only where a session actually ran. It cannot reach a repository that has no local checkout, which is most of what `all_visible` adds. Against a source running *here*, only the `ignore` class withholds a row: a `local-only` directory still produces an admitted local row whose `git_remote` is evidence. Against a *server* it does not, because `local-only` rows are withheld at the export seam and never reach a server at all, so they are never evidence for its capture. So point a user who wants a repository out of a **local** graph at the GitHub source's `ignore` list, and a user who wants it off the **team server** at `hyp privacy set <dir> local-only` (or `ignore`); neither is `.hypignore` as a blanket repository promise.
- **The evidence boundary is prospective.** A session dropped by `.hypignore` supplies no row of its own. Adding `.hypignore` or `ignore` does not erase previously admitted rows or already captured GitHub history.
