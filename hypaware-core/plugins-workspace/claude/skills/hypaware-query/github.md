...
- **Capture is agent-evidence-gated.** The server captures GitHub history only for configured repositories that appear as `git_remote` on an admitted `ai_gateway_messages` row. A session dropped by `.hypignore` supplies no row and cannot introduce its repository into a future GitHub backfill. This boundary is prospective: adding `.hypignore` does not erase previously admitted rows or already captured GitHub history.
