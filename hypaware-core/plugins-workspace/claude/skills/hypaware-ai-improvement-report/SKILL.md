...
IMPORTANT: Query server logs not local logs
...
2. **Scan the signals for possible improvements.** Work the three signals above; each turns
   ...
   - **Repeated work** — cluster sessions by shared-file overlap and tool-set signature into
     recurring kinds of work (reuse a recent `docs/spend-reviews/` *Leverage candidates*
...
- **Path:** `docs/improvement-reviews/<YYYY-MM-DD>-improvement-review.md` (create the dir
...
- **Then the detail**, in these sections: **Inventory (what exists) · Basis · Skill
  ...
  Efficiency → setup changes · Caveats** — plus every query you ran and the proposed
  AGENTS.md lines verbatim.
