...
IMPORTANT: Query the server, not local logs. **Descriptive only** — route every *action* out:
"fan out more/less" and tooling go to **hypaware-ai-improvement-report**, token waste to **hypaware-ai-spend-report**.
Query mechanics live in the **hypaware-query** skill; reuse hypaware-ai-spend-report's deduped token spine
for any token figure.
...
- **Path:** `docs/adoption-profiles/<YYYY-MM-DD>-adoption-profile.md` (create the dir if needed).
...
- **Capture-health note:** if subagent provenance doesn't reach the server, the standing #1
  caveat is "subagent identity must survive ingest" — run adoption off the `Task`-call proxy and
  flag it.
