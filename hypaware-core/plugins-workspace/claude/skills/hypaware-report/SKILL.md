...
to the workers. Use the host's subagent tool with the registered
`hypaware-analyst` agent type, including a plugin namespace if the host exposes
one. The HypAware Claude plugin supplies this worker. A skill describes this
orchestration; it does not register the worker or implement a tool itself.

Check that the named worker is available before starting the investigation.
If it is unavailable, explain the missing dependency. Do not silently replace
it with a generic worker or do all its reading in the coordinator. An alternate
agent implementation or a single-agent report requires the user's choice.
