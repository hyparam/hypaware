...
Use the host's subagent tool to launch the registered `hypaware-analyst` worker.
The agent type is a tool argument, not merely a name written in the task text.
Preserve the installed worker's model and tool restrictions. Do not copy an
agent definition into the user's configuration as part of generating a report.
