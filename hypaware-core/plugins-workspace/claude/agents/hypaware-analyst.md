...
- **Unfamiliar table?** Run `hyp query schema <table>` once, then query. Works for every dataset listed by `hyp query status`.
...
For exact columns in the installed version: `hyp query schema <table>`. For the full reference on `hyp query`, read `~/.claude/skills/hypaware-query/SKILL.md`.
