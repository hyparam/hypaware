...
export function createGithubClient({ tokenEnv, env, log, fetchImpl, baseUrl = API_BASE, ghToken, execFileImpl }) {
  ...
  async function request(pathAndQuery, opts = {}) {
    ...
    // that already appended this page then restarts from its unpublished
    // watermark on every later tick and re-appends the same rows, because
    // `github_events` dedupes within one batch and not across ticks.
