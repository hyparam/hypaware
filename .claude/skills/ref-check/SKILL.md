...
- **Every anchor the document defines.** An LLP names its sections three ways, and all three are anchors a `@ref` may target. Index all of them or you will report resolvable references as broken:
  ...
  Measured on this corpus (1355 references), indexing only heading slugs reports **632** resolvable references as BROKEN: 366 target a `{#...}` marker, 266 an inline `<a id>`. Getting the whitespace rule wrong on top of that adds 30 more. All of it matters, but the anchor *forms* are by far the larger share, and restricting `{#...}` to heading lines is worth 47 of them on its own. With all three forms indexed as described, **22** references are genuinely broken (issue #457), which is the number a run should reproduce.
