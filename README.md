...
- **On macOS, an earlier proxy attach also left a login-session variable
  ...
  must be fully quit and reopened. `hyp detach <client> --purge` and `hyp
  daemon uninstall` clear both unconditionally, and `hyp attach claude`
  unwinds them when it migrates a previously proxied machine. A plain `hyp
  detach <client>` only clears them while that client's attach marker still
